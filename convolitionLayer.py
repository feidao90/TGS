...
# 获取本地input_data
def input_data():
    g = os.walk('./resources/train/images')

    filepathlist = []
    for path, d, filelist in g:
        for filename in filelist:
            if filename.endswith('png'):
                image = img.imread(str(path+'/'+filename))
                filepathlist.append(image)
    return filepathlist

# 分离数据
def segment_data(list):
    result = dict()
    trainlist = np.random.choice(len(list),round(len(list)*0.8),replace=False)
    result['train'] = trainlist
    testlist = np.array(set(range(len(list))) - set(trainlist))
    result['test'] = testlist
    return result

# 获取本地train target data
def get_train_target():
    ...
    with open(path) as csvfile:
        ...
        for row in csv_reader:  # 将csv 文件中的数据保存到birth_data中
            data_dic = dict()
            data_dic['key'] = row[0]
            data_dic['value'] = row[1]
            target_train_list.append(data_dic)
    return target_train_list
